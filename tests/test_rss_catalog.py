...
# The raw core-IP JSONs remain local-only and are never published.
# Rebuild tests require them; the published repository runs the suite
# against knowledge/rss-catalog.json and skips the rebuild checks.
RAW_SOURCES_PRESENT = all(
    (REPO / rel).is_file() for rel in rss_catalog.CORE_IP_FILES
)
RAW_SOURCES_SKIP = (
    "raw core-IP JSONs are local-only; published repo tests use "
...
class CatalogExtractionTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        if not RAW_SOURCES_PRESENT:
            raise unittest.SkipTest(RAW_SOURCES_SKIP)
...
class CatalogFileTests(unittest.TestCase):
    def test_write_and_load_roundtrip(self):
        if not RAW_SOURCES_PRESENT:
            self.skipTest(RAW_SOURCES_SKIP)
