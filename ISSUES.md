...
当前阶段：审计记录完成；提示词逐项评审中；未实施本轮线上修复。
...
[KNOWN] 本次授权是记录问题并与编辑按 LCW 主线逐个评审提示词，不是批量部署此前优化包。候选提示词、Schema 和迁移建议均不能自动视为已批准或已上线。
...
| LCW-003 | P0 / REVIEWING | `Set Discovery Params1`、Narrative、Task Master、Writer、`Parse & Fan-out Tasks` | [KNOWN] 原审计附录及 E09：预设冲突、历史专题示例和代码内旧人设会继续塑造文章。 [INFERRED] 将分析维度改为调查问题，不预设厂商动机、丑闻或固定 CTO 结论；缺字段不能注入旧人格。当前仅审入口 `topic_intent`。 |
...
| 顺序 | 主线位置 | 待审内容 | 评审进度 |
| --- | --- | --- | --- |
| 01 | `Set Discovery Params1` | `topic_intent`：发现什么值得继续研究的事件 | REVIEWING；候选待编辑反馈 |
...
### 当前游标：01 / `Set Discovery Params1.topic_intent`

[KNOWN] 2026-09-20 审计记录的入口使用 `timeframe=72h`、`target_count=5`、`source_lane=auto`。早期规格中的“三个主题”不作为本次偷偷改发现条数的依据。

[INFERRED] 待讨论方向：保留读者价值、信息增量与跨维度调查能力；将垄断、内斗、掩盖财报等预设改成需证据回答的问题；不因材料来自官方就整体排除，也不把官方效果声明当成独立实测。此项只审 `topic_intent`，暂不替换 `triage_prompt` 或改动发现数量、调度、接口。

[INFERRED] 候选允许表达“最多返回目标数量，不为凑数虚构”；但实际启用少于五条的输出前，必须同步修复固定索引访问 0–4 的选题邮件/表单适配。相关接收逻辑未改之前，不把该文本称为可直接部署的无依赖替换。

[KNOWN] 当前未记录任何提示词的用户批准、生产应用或新版成稿验收。
...
[INFERRED] 新模型生成或全篇 E2E 不因更新本文件而自动获准；测试备注、来源内容与系统指令必须隔离。历史绿色执行记录、静态 Schema 检查和模型声称“完成”都不能单独关闭编辑质量问题。
