...
1. **News Peg（导语）**：第一段直接抛出决定性结论或最清晰的画面；再给最近的
   硬事实，不寒暄、不铺垫。
...
- **句长弹性**：大多数句子控制在 25 词以内；为转折、因果或解释可保留少量长句，
  不能把所有句子机械切成同一长度。
...
- **口语化**：用缩写（didn't / can't / won't）与少量句子碎片制造爆发力；连续独立
  短句最多两段，不能把全文写成电报。
...
- **意象先于术语**：先给可看见的画面（"it can watch the cost of each
  ...
  一处五感/场景细节。选择一个主导意象并回扣它；禁止堆叠门、账本、邮箱等
  互不相干的比喻。
- **节奏多样性**：全文通常 18–24 段，段落长短交替；独立 punch line 连续不超过
  两段；禁止多段同构
  （加粗断言+限定+链接的节拍器）。
