...
def _compile_prompt(article: str, evidence: dict) -> str:
    ...
    return (
        ...
        "Scope rules — do NOT flag these as unsupported:\n"
        "1. General domain background the article explicitly frames as "
        "background (e.g. 'the conventional mechanism', 'usually', "
        "'typically', 'describes how X works'); background knowledge is "
        "not an audited claim about this specific event.\n"
        "2. The article's own absence/hedge statements when they match "
        "the evidence package: 'could not be independently reviewed', "
        "'no raw benchmark tables', 'not independently confirmed', 'no "
        "independent reproduction' — verify they are true of the evidence "
        "(missing body, second-hand status, no such source) and mark them "
        "ok.\n"
        "3. Meta-statements about source level ('vendor-reported', "
        "'second-hand', 'relayed', 'single source') — correct labeling, "
        "not errors.\n"
        "Only flag a claim unsupported when the article asserts a "
        "specific fact about this event as certain and the evidence "
        "contradicts it or lacks it entirely.\n"
