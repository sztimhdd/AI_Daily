...
def build_plan_prompt(article: str, evidence: dict) -> str:
    ...
    return (
        ...
        "User-preferred cover direction: editorial pen-and-ink magazine "
        "illustration. Give this family priority for creative tools, "
        "computer use, human agency, and accessible technology stories "
        "when a concrete visual metaphor fits. It is a preference, not "
        "a mandatory style for every topic; retain the full library for "
        "stories better served by other visual languages.\n"
        "When selecting this direction, explicitly include in the image "
        "prompt: warm ivory paper, confident black contour lines, selective "
        "fine hatching, human hand-drawn imperfections, generous negative "
        "space, and one or two restrained spot colors (for example muted "
        "vermilion and dusty blue). Favor a tactile, witty, inviting "
        "culture-and-technology magazine illustration over a sterile "
        "diagram or photorealistic scene. Use one immediately readable "
        "action to express the article's idea. A previously liked example "
        "was a cursor becoming a hand that paints outside a monitor, with "
        "piano keys as a secondary clue to computer use. Learn its clear "
        "metaphor and economical linework, NOT its specific objects: do "
        "not repeat the monitor, hand, brush, or piano unless this article "
        "calls for them. Avoid stock desk clutter, gratuitous flowers, "
