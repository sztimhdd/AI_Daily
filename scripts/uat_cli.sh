...
check "package article named by slug" test -f "$PKG/$SLUG.md"
...
check "final article identical to package copy" cmp -s "$PKG/$SLUG.md" "$FINAL"
check "draft has H1" grep -q "^# " "$PKG/$SLUG.md"
check "draft carries source links" grep -q "https://" "$PKG/$SLUG.md"
check "no unresolved image placeholders" bash -c "! grep -q '{\[IMG' '$PKG/$SLUG.md'"
check "no n8n leftovers" bash -c "! grep -qi 'n8n' '$PKG/$SLUG.md'"
